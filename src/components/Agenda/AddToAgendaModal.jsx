...
function AddToAgendaModal({ isOpen, onClose, task }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-surface rounded-xl shadow-2xl w-full max-w-sm mx-4">
        <div className="flex items-center justify-between p-4 border-b border-status-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-cell-primary">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          {/* Task name preview */}
          <div className="p-3 bg-status-bg rounded-lg">
            <p className="text-sm font-medium text-cell-primary truncate">
              {task?.title || task?.name}
            </p>
            {task?.description && (
              <p className="text-xs text-cell-secondary mt-0.5 line-clamp-2">
                {task.description}
              </p>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2 text-sm font-medium text-cell-secondary hover:bg-status-bg rounded-lg transition-colors"
            >
              {t("Cancel")}
            </button>
            <button
