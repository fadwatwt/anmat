      ...
      {isGated ? (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-950/20 dark:to-orange-950/20 border border-red-200 dark:border-red-900/50 rounded-2xl shadow-lg px-6 py-4 max-w-3xl mx-auto backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-cell-primary font-medium text-sm sm:text-base text-center sm:text-start">
        ...
        <form className="flex items-center gap-2 bg-surface rounded-2xl shadow-lg px-4 py-3 border border-status-border max-w-3xl mx-auto" onSubmit={handleSubmit}>
          {isRecording ? (
            ...
            <div className="flex items-center gap-3 w-full">
              ...
              <div className="flex-1 h-2 bg-status-bg rounded-full overflow-hidden">
            ...
            <div className="flex items-center gap-3 w-full">
              ...
              <span className="text-sm text-cell-secondary font-mono">{formatTime(recordingSeconds)}</span>
              <button
                type="button"
                onClick={onCancelAudio}
                className="p-2 rounded-full text-cell-secondary hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                title={t("Cancel")}
              >
                <Trash2 size={16} />
              </button>
            ...
            <>
              <button
                type="button"
                className={`text-cell-secondary hover:text-primary-500 p-2 transition-colors relative ${isRecording ? 'text-red-500' : ''}`}
              ...
              <button type="button" className="text-cell-secondary hover:text-primary-500 p-2 transition-colors" onClick={handleAttachmentClick}>
              ...
              <div className="flex-1 flex flex-col">
                <input
                  type="text"
                  ref={inputRef}
                  className="flex-1 rounded-lg border-none bg-transparent px-4 py-2 focus:outline-none focus:ring-0 dark:bg-transparent text-cell-primary placeholder-gray-500 dark:placeholder-gray-400"
                ...
                {stagedFiles.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2 px-4">
                    {stagedFiles.map((file, index) => (
                      <div key={index} className="flex items-center bg-status-bg rounded-full pl-3 pr-2 py-1 text-sm text-cell-secondary">
                        {file.preview && (
                        ...
                        <button
                          ...
                          className="ms-1 p-1 rounded-full hover:bg-status-bg dark:hover:bg-gray-600"
                        >
                          <X size={14} />
                        </button>
                      </div>
        ...
        <div className="flex items-center justify-center gap-2 mt-2 max-w-3xl mx-auto">
          {quickActions.map((qa, i) => (
            <button
              key={i}
              type="button"
              className="text-xs text-cell-secondary hover:text-primary-500 dark:hover:text-primary-400 hover:bg-status-bg px-3 py-1.5 rounded-full border border-status-border transition-colors"
