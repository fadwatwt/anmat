...
function LeavesTab() {
    ...
    const confirmDelete = async () => {
        showProcessing(t("Deleting Short Leave Record..."));
        try {
            await deleteLeave(selectedLeave._id).unwrap();
            setDeleteApiResponse({
                isOpen: true,
                status: "success",
                message: t("Short Leave record deleted successfully")
            });
        } catch (error) {
            setDeleteApiResponse({
                isOpen: true,
                status: "error",
                message: error?.data?.message || error.message || t("Failed to delete short leave record")
            });
        } finally {
            hideProcessing();
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <ApprovalAlert
                ...
                title={t("Delete Short Leave Record")}
                message={t("Are you sure you want to delete this short leave record? This action cannot be undone.")}
