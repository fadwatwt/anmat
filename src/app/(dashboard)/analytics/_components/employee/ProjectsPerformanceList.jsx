        ...
        <ContentCard
            ...
            main={
                <div className="flex flex-col gap-8 h-full">
                    {projects.length === 0 && (
                        <span className="text-sm text-cell-secondary">{t("No projects assigned.")}</span>
                    )}
                    {projects.map((project, index) => {
                        ...
                        return (
                            <div key={index} className="flex items-start gap-4 justify-between flex-col md:flex-row">
                                <div>
                                    <h3 className="text-md font-medium text-cell-primary">{project.name}</h3>
                                </div>
                                <div className="flex flex-col gap-1 w-full md:w-3/4">
                                    <div className="w-full bg-status-bg rounded-full h-2.5">
                                        <div
                                    ...
                                    <div className="flex flex-wrap gap-1 items-start justify-between w-full">
                                        <p className="text-sm text-cell-secondary">
                                            {t("{{completed}}/{{total}} tasks completed", { completed: project.completedTasks ?? 0, total: project.totalTasks ?? 0 })}
                                        </p>
