        ...
        return rowsProp.map((dept) => [
            <React.Fragment key={dept.rank + dept.name}>
                <div className={"flex justify-start items-center gap-2"}>
                    <div className={"flex justify-center items-center rounded-full w-9 h-9 border border-gray-400"}>
                    ...
                    <div className={"flex flex-col items-start gap-1"}>
                        ...
                        <p className={"text-sm text-cell-secondary"}>{t("Manager:")} {dept.manager}</p>
                    </div>
                </div>
            </React.Fragment>,
            String(dept.rank),
        ...
        <div className={"w-full"}>
            <div className="flex items-center gap-2 mb-2">
                ...
                <button
                    ...
                    className="p-1 rounded-full hover:bg-status-bg transition-colors flex-shrink-0"
                    title={t("How scoring works")}
                >
                    <RiInformationLine size={20} className="text-cell-secondary" />
                </button>
            </div>
            ...
            {showInfo && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-surface rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-status-border">
                            <h3 className="text-lg font-bold text-cell-primary">{t("How scoring works")}</h3>
                            <button
                                onClick={() => setShowInfo(false)}
                                className="p-1 rounded-full hover:bg-status-bg transition-colors"
                            >
                                <RiCloseLine size={20} className="text-cell-secondary" />
                            </button>
                        </div>
                        <div className="px-6 py-5 space-y-4">
                            {scoringItems.map((item, index) => (
                                <div key={index} className="flex gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold text-cell-primary mb-0.5">{item.label}</p>
                                        <p className="text-sm text-cell-secondary leading-relaxed">{item.text}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className="px-6 py-3 border-t border-status-border flex justify-end">
                            <button
                                onClick={() => setShowInfo(false)}
