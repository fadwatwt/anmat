...
const SavedMethodCard = ({ method, isSelected, onSelect, onEdit }) => {
    ...
    return (
        <div
            className={`relative flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all flex-1 ${isSelected ? 'border-blue-600 dark:border-blue-800' : 'border-status-border bg-surface'}`}
            onClick={() => onSelect(method.id)}
        >
            <div className="flex items-center gap-4 w-full">
                {/* Radio Button */}
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-blue-600 dark:border-blue-800' : 'border-status-border'}`}>
                    {isSelected && <div className="w-2.5 h-2.5 bg-blue-600 rounded-full" />}
                </div>

                {/* Info */}
                <div className="flex-1">
                    <p className="font-bold text-cell-primary">**** {method.last4}</p>
                    <div className="flex items-center gap-2 text-sm text-cell-secondary">
                        <span>{method.type}</span>
                        <span>•</span>
                        <button
                            onClick={(e) => { e.stopPropagation(); onEdit(method.id); }}
                            className="hover:underline text-cell-secondary"
                        >
                            {t('Edit')}
                        </button>
                    </div>
...
function SavedMethodsGroup({ methods, selectedId, onSelect, onEdit, onAddNew }) {
    ...
    return (
        <div className="flex flex-col gap-4 w-full">
            <h3 className="text-cell-primary">{t('Payment method')}</h3>
