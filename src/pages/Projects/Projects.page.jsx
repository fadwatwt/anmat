...
function ProjectsPage() {
  ...
  const ProjectRowTable = () => {
    return projects.map((project) => [
      <>
        <NameAndDescription
          path={`/projects/${project.id}-${convertToSlug(project.name)}`}
          id={"4"}
          name={project.name}
        ...
        <div className={"flex flex-col gap-6"}>
          <div className="flex flex-col gap-2 h-full">
            <Table
              ...
              title={"All projects"}
              headers={headers}
              handelDelete={handelDeleteProject}
              handelEdit={handelEditModal}
              isActions={true}
              rows={rows}
              isFilter={true}
            />
